#任务表
#TODO 限制areaid跑北京市的数据
import time
from pymongo import MongoClient
import pymysql
from Mongo2Sql5 import getAllRegionAndCreateRecord
from Mongo2Sql6 import queryDocByPeriodAndOrg6
from Mongo2Sql7 import queryDocByPeriodAndOrg7
from Mongo2Sql8 import queryDocByPeriodAndOrg8
from UtilTool import str2PureNum

db_name = 'ggj-working'
collection_sum_name = 'stats_working_integrate_task'
collection_base_name = 'stats_working_gathered_respondent_task'
...
collectionSum = database[collection_sum_name]
collectionBase = database[collection_base_name]
collectionReport = database[collectionReportName]
#连接mysql数据库  后面加上 charset="utf8mb4" 这样方便传输汉字，解决字符集不匹配的问题
clientSql = pymysql.connect(host='192.168.6.203',port=3308, user='root',password='root',database='uap_data',charset='utf8mb4')
...
#     #4条任务中每条任务都对应综
#如果执行失败想从失败id开始执行
#collectionSum.find("locale": "zh", numericOrdering:true},).sort("_id", -1).batch_size(10000)
#({"locale": "zh", numericOrdering:true,'$where':'this._id < '}).sort("_id", -1)
res_cussor = collectionSum.find().sort([('id',-1)]).limit((id_index-1)*id_length).batch_size(10000)
for i in res_cussor: #or collectionBase.find().batch_size(10000):
    #mongo的组织机构代码对应mysql:org_id
    ...
    timestamp = time.localtime(i['fillDeadline']/1000)
    ...
    for j in range(4):
        ...
        try:
                ...
                #5表操作:5表生成1条recordid对应多2条5表里的数据
                if(form_id == 10006):
                    getAllRegionAndCreateRecord(id,form_id,form_name,type,org_id,org_name,informant_id,informant,sta_principal_id,sta_principal,org_principal_id,org_principal,status,state,is_del,sta_year,sta_month,sta_season,report_date,created_time,updated_time,warnning_code,warning_remark,audit_remarks,mobile)
                #6表操作
                if(form_id == 10007):
                    queryDocByPeriodAndOrg6(periodId,org_id,record_id,sta_year)
                #7表操作
                if(form_id == 10008):
                    queryDocByPeriodAndOrg7(periodId,org_id,record_id,sta_year)
                #8表操作
                if(form_id == 10009):
                    queryDocByPeriodAndOrg8(periodId,org_id,record_id,sta_year)
                cursor.execute(sql, par)
                clientSql.commit()
        except Exception as e:
                clientSql.rollback()
