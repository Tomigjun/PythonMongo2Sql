...
sql = 'INSERT INTO {} (record_id,p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,p17,p18,p19,p20,p21,p22) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
...
def queryDocByPeriodAndOrg4(periodId,orgId,recordId,periodYear):
    res = []
    hisTableName = tableName+subfix+periodYear
    cursor4 = clientSql4.cursor()
    query = {"periodId": periodId, "respondentId": orgId,'reportCode': 'NY203_2020'}
    collectionData = database[collectionReportName]
    # count = collectionData.count(query)#.sort("info.usage_count_180_day", -1).limit(number)
    # if(count == 0):
    #     return
    res_cussor4 = collectionData.find(query)
    # res_data4 = list(res_cussor4[:])
    for i in res_cussor4:
        keyNotExistValueSetParam(i['rowData'])
        par = (recordId,p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,p17,p18,p19,p20,p21,p22)
        try:
