...
# id =  100001
id_length = 300000;
id = (id_index -1)*id_length*4 + 1
# id = 1166383*4 + 1
#report_record表同步
sql = "INSERT INTO report_record (id,form_id,form_name,`type`,org_id,org_name,informant_id,informant,sta_principal_id,sta_principal,org_principal_id,org_principal,status,state,is_del,sta_year,sta_month,sta_season,report_date,created_time,updated_time,warnning_code,warning_remark,audit_remarks,mobile,period_id) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
...
res_cussor = collectionTaskBase.find().sort([('_id',-1)]).limit(id_length).skip((id_index-1)*id_length).batch_size(10000)
# res_cussor = collectionTaskBase.find().sort([('_id',-1)]).limit(33617).skip(1166383).batch_size(10000)
...
for i in res_cussor:
    ...
    for j in range(1,5):
        ...
        form_id =10001 +j
        record_id = id
        par = (id,form_id,form_name,type,org_id,org_name,informant_id,informant,sta_principal_id,sta_principal,org_principal_id,org_principal,statusRes,state,is_del,sta_year,sta_month,sta_season,report_date,created_time,updated_time,warnning_code,warning_remark,audit_remarks,mobile,periodId)
        try:
            #
            # #1表操作
            # if(form_id == 10002):
